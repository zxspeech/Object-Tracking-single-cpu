...
const double adaptThres = 0.5;  //0.1 results in a larger RMES. 0.5 -> RMSE:1.59862.
...
//    const String filename = "./debate.mov"2
...
//                search_spaceColor, NumofparticlesPerCore, 2, 10, 50); //for debate.
    ...
    particleFilter2DColor tracker = particleFilter2DColor(colorFace, x, y, frame,
            search_spaceColor, NumofparticlesPerCore, numProcs, 2, 20, 22, adaptThres); //for ducklings.
...
//    if(0 == myRank){
//        std::cout<<"trakcer.th: "<<tracker.th<<std::endl;
//    }
...
while(true) {
    ...
    //        MPI_Barrier(MPI_COMM_WORLD);
         if (0 == myRank) {
             ...
             tracker.calNeff();
             std::cout<<"Neff: "<< tracker.Neff <<std::endl;
...
//     showPairVectorContents(tracker.particle);
 ...
 //    showPairVectorContents(tracker.particle);
    ...
    else if(0 != tracker.timeStep % timeInterval){
        LOCAL:
//        MPI_Barrier(MPI_COMM_WORLD);
        ...
        if(0 == myRank){
            if(1 == tracker.skipGlobal){
                std::cout<<"Enter Local from skipping global "<< std::endl;
            }
            else
